...
public class DisplayKeycode : MonoBehaviour
{
    ...
    #region Razer Keyboard Mappings
    ...
    void Update()
    {
        if (_mLastKey != KeyCode.None &&
            !Input.GetKey(_mLastKey))
        {
            if (_mTextLabel)
            {
                _mTextLabel.text = "KEY RELEASED";
            }
            if (_mTextKeyCode)
            {
                _mTextKeyCode.text = _mLastKey.ToString();
            }
            if (_mTextRazerKeyCode)
            {
                _mTextRazerKeyCode.text = ((Keyboard.RZKEY)GetKeyboardRazerKey(_mLastKey)).ToString();
            }
            _mLastKey = KeyCode.None;
        }
        ...
        foreach (KeyCode vKey in System.Enum.GetValues(typeof(KeyCode)))
        {
            if (Input.GetKey(vKey))
            {
                if (Input.GetKeyDown(vKey))
                {
                    ...
                    if (_mTextLabel)
                    {
                        _mTextLabel.text = "KEY PRESSED";
                    }
                    if (_mTextKeyCode)
                    {
                        _mTextKeyCode.text = vKey.ToString();
                    }
                    if (_mTextRazerKeyCode)
                    {
                        _mTextRazerKeyCode.text = ((Keyboard.RZKEY)GetKeyboardRazerKey(vKey)).ToString();
                    }
                }
            }
        }
